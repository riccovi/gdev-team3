...
public class Wrench : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Add a condition to handle when the wrench is damaged
        if (isDamaged)
        {
        // Perform some action when the wrench is damaged
        Debug.Log("Wrench is damaged!");
        
        // Reset the damage state if necessary
        isDamaged = false; // or leave it true if you want to handle it elsewhere
        }
    ...
    private IEnumerator ThrowWrenchCoroutine(Vector3 targetPosition)
    {
        ...
        while (Vector2.Distance(transform.position, targetPosition) > 0.2f)
        {
            ...
            // Check if the wrench is about to change direction
            if (!hasChangedDirection && Vector2.Distance(transform.position, targetPosition) < 0.2f)
            {
                hasChangedDirection = true;
                // Add your logic here when the wrench changes direction
                Debug.Log("The wrench has changed direction!");
            }    
